...
const CONFIG = {
  ...
  ADMIN_KEY: process.env.ADMIN_KEY || "", // секретный ключ для страницы /admin.html — придумай свой в .env!
};
...
/* ==========================================================================
   АДМИНКА — просмотр и обработка заявок на вывод.
   Открывается на /admin.html. Защищена секретным ключом ADMIN_KEY (задаётся
   в переменных окружения) — это не пароль Telegram-пользователя, а отдельный
   ключ только для тебя, владельца проекта. Никому не давай ссылку с ним.
   ========================================================================== */
function checkAdminKey(req, res) {
  const key = req.query.key || req.body?.key;
  if (!CONFIG.ADMIN_KEY) {
    res.status(500).json({ error: "admin_key_not_configured" });
    return false;
  }
...
app.post("/api/admin/withdrawals/:id/status", (req, res) => {
  ...
  const result = db.setWithdrawalStatus(req.params.id, status);
