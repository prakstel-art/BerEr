...
// Отдаём фронтенд (index.html и всё остальное из папки public) с того же
// сервера и того же адреса, что и API. Благодаря этому не нужен отдельный
// GitHub Pages — один деплой на bothost поднимает и сайт, и API сразу.
const PUBLIC_DIR = path.join(__dirname, "public");
const INDEX_FILE = path.join(PUBLIC_DIR, "index.html");

if (!fs.existsSync(INDEX_FILE)) {
  console.error("=".repeat(60));
  console.error(`ВНИМАНИЕ: не найден файл ${INDEX_FILE}`);
  console.error("Проверь, что папка 'public' с index.html внутри лежит");
  console.error("рядом с server.js (в корне проекта), а не где-то ещё.");
  console.error("=".repeat(60));
} else {
...
// Явный запасной маршрут на "/", если по какой-то причине express.static
// не сработал (например, файл лежит не совсем там, где ожидается) —
// тогда вместо непонятного "Cannot GET /" будет понятная подсказка.
app.get("/", (req, res) => {
  if (fs.existsSync(INDEX_FILE)) {
    res.sendFile ? res.sendFile(INDEX_FILE) : res.send(fs.readFileSync(INDEX_FILE, "utf8"));
  } else {
    res.status(500).send(
      "index.html не найден на сервере. Проверь, что папка public/ с " +
      "index.html внутри загружена в репозиторий рядом с server.js."
    );
  }
});
